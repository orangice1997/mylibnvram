...
char *nvram_get(const char *key1, const char* key2) {
...
// #endif
    ...
    return (nvram_get_buf(key2, temp, BUFFER_SIZE) == E_SUCCESS) ? strndup(temp, BUFFER_SIZE) : strdup("");
}

char *nvram_safe_get(const char *key) {
    char* ret = nvram_get(key, key);
